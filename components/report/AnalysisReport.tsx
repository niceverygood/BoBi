            ...
            >
                ...
                {analysisResult && (
                    <section style={{ marginBottom: '28px' }}>
                        ...
                        {/* Overall Summary */}
                        <div style={{ background: '#f0f4ff', padding: '12px 16px', borderRadius: '8px', marginBottom: '16px', border: '1px solid #dbeafe' }}>
                            <p style={{ margin: 0, fontSize: '12px', color: '#1e3a5f' }}>
                        ...
                        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '11px', marginBottom: '12px' }}>
                            <thead>
                                <tr style={{ background: '#f8fafc' }}>
                                    <th style={thStyle}>고지 항목</th>
                                    <th style={{ ...thStyle, width: '60px', textAlign: 'center' }}>해당여부</th>
                            ...
                            <tbody>
                                {analysisResult.items.map((item, i) => (
                                    <tr key={i}>
                                        <td style={{ ...tdStyle, width: '250px' }}>{item.question}</td>
                        ...
                        {analysisResult.riskFlags && analysisResult.riskFlags.length > 0 && (
                            <div style={{ marginTop: '8px' }}>
                                ...
                                {analysisResult.riskFlags.map((flag, i) => (
                                    <div key={i} style={{ fontSize: '11px', marginBottom: '4px', padding: '4px 8px', background: '#fef2f2', borderRadius: '4px', border: '1px solid #fecaca' }}>
                        ...
                        {analysisResult.diseaseSummary && analysisResult.diseaseSummary.length > 0 && (
                            <div style={{ marginTop: '12px' }}>
                                ...
                                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '10px' }}>
                                    <thead>
                                        <tr style={{ background: '#f8fafc' }}>
                                            <th style={thSmallStyle}>질병명</th>
                                            <th style={thSmallStyle}>코드</th>
                                            <th style={thSmallStyle}>기간</th>
                                            <th style={thSmallStyle}>총방문</th>
                                            <th style={thSmallStyle}>상태</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                        ...
                        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '11px', marginBottom: '12px' }}>
                            <thead>
                                <tr style={{ background: '#f8fafc' }}>
                                    ...
                                    <th style={{ ...thStyle, width: '60px', textAlign: 'center' }}>가입가능</th>
                                    <th style={thStyle}>추천사항</th>
                                </tr>
                            </thead>
                            <tbody>
                    ...
                    <section style={{ marginBottom: '28px' }}>
                        ...
                        {claimResult.claimableItems && claimResult.claimableItems.length > 0 && (
                            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '10px', marginBottom: '12px' }}>
                                <thead>
                                    <tr style={{ background: '#f8fafc' }}>
                                        <th style={thSmallStyle}>진료일</th>
                                        <th style={thSmallStyle}>병원</th>
                                        <th style={thSmallStyle}>진단</th>
                                        <th style={thSmallStyle}>유형</th>
                                        <th style={thSmallStyle}>청구결과</th>
                                    </tr>
                                </thead>
                                <tbody>
                        ...
                        <div style={{ background: '#f5f3ff', padding: '12px 16px', borderRadius: '8px', border: '1px solid #ddd6fe' }}>
                            <p style={{ margin: 0, fontSize: '12px' }}><strong>종합:</strong> {claimResult.summary}</p>
                        </div>
                        ...
                        {claimResult.importantNotes && claimResult.importantNotes.length > 0 && (
                            <div style={{ marginTop: '8px', fontSize: '11px' }}>
                                ...
                                {claimResult.importantNotes.map((note, i) => (
                                    <div key={i} style={{ padding: '2px 0', color: '#666' }}>• {note}</div>
                                ))}
                            </div>
                        )}
                    </section>
