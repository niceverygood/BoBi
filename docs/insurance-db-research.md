...
전략 트랙 4가지(A 직접 크롤링 / B 마이데이터 인가 / C 제휴 / D 하이브리드)를
비교한 결과, **(D) 하이브리드 — "안전한 출처만 직접 크롤링 + 마이데이터 사업자
제휴 병행"** 을 권장. Bottle Inc.의 자본·시간 제약 + 합법성 리스크 분산이 근거.
상세 비교표·근거는 **§7.0**.

(D) 채택 시:
- 첫 6주는 **금감원 fine.fss.or.kr의 정적 비교공시 페이지 + 보험사 IR/공시 PDF**만 자동 수집하고, 협회 공시는 보름 안에 협조 요청 메일을 보낸다. 명시 차단 사이트(생보협회·DB손보 등)는 자동 제외.
- 동시에 한승수 대표가 마이데이터 제휴(뱅크샐러드/카카오페이) 협상을 진행 — 1~3개월 트랙.
- 인프라는 **Python + Playwright + GitHub Actions 일 1회 cron + Supabase `insurance` 별도 스키마** (보비 본체와 권한·RLS·키 모두 분리).

상세는 §6 Phase 2 추정 + §7 결정 항목.
...
§1~§6은 "직접 크롤링" 트랙(A)을 전제로 작성됐다. 그러나 Bottle Inc.의
자본 상태·합법성 리스크·6주 시간 제약을 함께 보면 **트랙 자체를
재선택할 여지**가 있다. 여기서 결정하지 않으면 §7.1~§7.5는 의미가 없다.

| 옵션 | 자본금 요건 | 합법성 리스크 | 데이터 깊이·신뢰도 | Phase 2 시간 | 운영 부담 |
|---|---|---|---|---|---|
| **(A) 직접 크롤링** (본 보고서 본안) | 0 | 🔴 높음 (생보협회 명시 차단·403 사이트 다수) | 🟡 중 — 공시 데이터 위주, 인수기준은 거의 못 가짐 | 6주 | 🔴 높음 — 사이트 개편 빈도, 컴플레인 대응 |
| **(B) 마이데이터 인가 직접 취득** | **자본금 5억** + 보안·인력 요건 | 🟢 낮음 | 🟢 가장 깊음 (사용자 동의 기반 정식 채널) | **6개월~1년** (인가 발급) | 🟡 중 |
| **(C) 마이데이터 사업자 제휴** (뱅크샐러드·카카오페이 등) | 0 (제휴 비용만) | 🟡 계약 의존 — 제휴사 ToS·재배포 권한 협상 | 🟡 제휴사 데이터 한정 (인수기준 없음) | 1~3개월 (계약·연동) | 🟢 낮음 — 제휴사가 운영 |
| **(D) 하이브리드 (A 일부 + C)** ★ **추천** | 0 | 🟡 중 — 명시 차단 사이트는 제외, 안전한 출처만 | 🟡 중 → C 제휴 후 🟢로 향상 | 6주 (A) + 1~3개월 (C) 병행 | 🟡 중 |

### 권장: (D) 하이브리드 ★

근거 4가지:
1. **자본 제약** — Bottle Inc.가 마이데이터 자본금 5억 요건을 단기에
   충족하기 어려운 상태로 추정. (B) 단독 트랙은 시간상 비현실적.
2. **합법성 분산** — (A)에서 robots 명시 차단된 생보협회·DB손보·KB손보는
   완전 제외하고, 공익성 명확한 금감원·삼성화재·교보생명 등 "안전한
   출처만" 크롤링. 위반 리스크 사이트는 (C) 제휴사를 통한 정식 채널로
   대체.
3. **시간 분산** — (D)는 "지금부터 6주 안에 (A) 안전 사이트만으로
   1차 출시" + "병행으로 (C) 제휴 협상 1~3개월 진행" 구조.
   (B) 인가 1년을 기다리지 않고 사용자 가치를 먼저 만든다.
4. **(C) 제휴는 (B) 인가 트랙과 병행 가능** — 제휴사 트랙으로 운영
   경험을 쌓으면서 자본 확충·인가 준비를 따로 진행하면, 1년 후
   (B)로 자연스럽게 전환할 수 있다.
...
| 트랙 | 담당 | 시작 시점 | 산출물 |
|---|---|---|---|
| (A-제한) 안전 출처 크롤링 | 본 보고서 §6 일정대로 진행, 단 우선순위 5사를 §6.4 권장안으로 한정 | Phase 2 진입 즉시 | `crawler/sources/` 안의 fetcher들. 6주 내 출시. |
| (C) 마이데이터 제휴 협상 | 한승수 대표 직접 (BD) | Phase 2와 동시 | 제휴 LOI → 계약 → API 연동 (Phase 3) |
| (B) 마이데이터 인가 자본 확충 | 한승수 대표 (별도 트랙) | 분기 단위 검토 | 인가 신청은 6~12개월 후 |

### 7.1 변호사 자문 일정 (필수)

- §4.2의 4가지 항목 + (D) 트랙 채택 시 **(C) 제휴 계약 검토 항목**을 함께 자문
- Phase 2 진입 전 1회 컨설팅 (자문료·시간 사전 조율)

### 7.2 생보협회 공시(`pub.insure.or.kr`) 처리

- (D) 트랙에서 **자동으로 직접 크롤링 대상 제외**.
- 데이터 갭을 메우는 채널 결정: ① 협회 협조 메일 → ② (C) 제휴사 데이터로 일부 대체 → ③ 약관 PDF 일괄 수동 다운로드(주 1회)

### 7.3 신규 레포지터리 분리 vs 한 레포 유지

- 본 보고서는 한 레포 유지로 작성됨. (D) 트랙에서 코드 분량이
  Phase 2 종료 시점에 모니터링하다가, 6주 후 검토.

### 7.4 약관 PDF 저장소

- Supabase Storage / Cloudflare R2 / S3 중 선택. R2 권장(egress 무료).

### 7.5 첫 6주 우선순위 5사 확정

- §6.4 추천안 그대로 진행할지: 금감원 → 삼성화재·한화·교보·미래에셋·삼성생명
...
**이상.** Phase 2 진입 결정은 **§7.0 전략 트랙(A/B/C/D) 선택**을 가장 먼저 확정하고, 그 다음 §7.1~§7.5를 한승수 대표가 답한 뒤로 미룬다.
